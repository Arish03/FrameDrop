...
export default function LoginPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-64.8px)] w-full overflow-hidden bg-white selection:bg-black selection:text-white">
      ...
      <div className="hidden lg:flex flex-col flex-1 relative overflow-hidden bg-gray-50 border-r border-border/50">
        ...
        <div className="relative z-10 flex flex-col justify-center h-full px-20">
          ...
          <p className="text-black/40 text-lg leading-relaxed max-w-md mb-12 font-medium">
            The world's premier platform for museum-grade minimalist posters. Every frame, a masterpiece. Every wall, a gallery.
          </p>

          {/* Feature Chips */}
          <div className="flex flex-wrap gap-3">
            {[
              { label: "Museum Grade", icon: Shield },
              { label: "Fast Shipping", icon: Zap },
              { label: "Limited Drops", icon: Sparkles },
      ...
      {/* Login Panel */}
      <div className="flex flex-col items-center justify-center w-full lg:w-[500px] xl:w-[560px] flex-shrink-0 px-6 py-12 relative z-10 overflow-y-auto no-scrollbar">
        <div className="w-full max-w-xs p-8 border border-border/50 shadow-2xl relative overflow-hidden group bg-white">
          {/* Top Glow bar */}
          ...
          <div className="flex items-center gap-3 mb-10 lg:hidden justify-center">
            <span className="text-black text-2xl font-black tracking-widest uppercase">F.</span>
          </div>

          {/* Heading */}
          <div className="mb-8 text-center lg:text-left">
            <h2 className="text-black text-2xl font-black uppercase tracking-tighter mb-1">Access</h2>
            <p className="text-black/30 text-[10px] font-bold uppercase tracking-widest leading-relaxed">Member Portal</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            <div className="space-y-2">
              <label className="text-[9px] font-black text-black/40 uppercase tracking-[0.2em] block">Identifier</label>
              <div className="relative group/input">
                ...
                <input
                  ...
                  required
                  autoFocus
                  className="w-full bg-black/[0.02] border border-black/5 text-black focus:border-black/20 focus:ring-1 focus:ring-black/5 transition-all outline-none pl-11 pr-4 h-12 text-xs font-medium"
                />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative group/input">
                <Lock className="absolute left-4 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-black/20 transition-colors group-focus-within/input:text-black" />
                <input
                  ...
                  placeholder="••••••••"
                  required
                  className="w-full bg-black/[0.02] border border-black/5 text-black focus:border-black/20 focus:ring-1 focus:ring-black/5 transition-all outline-none pl-11 pr-11 h-12 text-xs font-medium tracking-widest"
                />
                <button
                  type="button"
            ...
            <Button
              type="submit"
              disabled={isLoading}
              className="w-full h-12 bg-black text-white hover:bg-black/90 font-black uppercase tracking-[0.15em] text-[10px] group flex items-center justify-center gap-3 transition-all duration-300 active:scale-95"
            >
              {isLoading ? (
                <div className="h-4 w-4 border-2 border-white/20 border-t-white rounded-full animate-spin" />
              ) : (
                <>Enter <ArrowRight className="h-3.5 w-3.5 transition-transform group-hover:translate-x-1.5" /></>
              )}
            </Button>
          </form>

          <div className="mt-8 pt-6 border-t border-border/50 flex flex-col gap-3 text-center">
             <Link href="/auth/register" className="text-[9px] text-black/40 hover:text-black uppercase font-black tracking-widest transition-colors decoration-black/20 underline underline-offset-4">
                Create Account
             </Link>
             <Link href="/" className="text-[9px] text-black/20 hover:text-black transition-colors uppercase font-bold tracking-widest">
                Exit
             </Link>
          </div>
        </div>
